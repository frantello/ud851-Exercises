...
public class AddTaskActivity extends AppCompatActivity {
    ...
    public void onClickAddTask(View view) {
        ...
        values.put(TaskContract.TaskEntry.COLUMN_DESCRIPTION, taskDescription.getText().toString());
        values.put(TaskContract.TaskEntry.COLUMN_PRIORITY, mPriority);

        Uri taskWithIdUri = getContentResolver().insert(TaskContract.TaskEntry.CONTENT_URI, values);

        // COMPLETED (8) Display the URI that's returned with a Toast
        // [Hint] Don't forget to call finish() to return to MainActivity after this insert is complete
        if (taskWithIdUri != null) {
            Toast.makeText(this, taskWithIdUri.toString(), Toast.LENGTH_LONG).show();
        }
